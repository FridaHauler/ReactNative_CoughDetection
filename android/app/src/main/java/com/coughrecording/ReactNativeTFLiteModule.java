package com.brainlab.coughmodel;
...
public class ReactNativeTFLiteModule extends ReactContextBaseJavaModule{
...
public ReactNativeTFLiteModule(ReactApplicationContext reactContext) {
	super(reactContext);
...
return "TFLiteReactNative Cough detection Try";}

@ReactMethod
private void loadModel(final String modelPath, final String labelsPath, final int numThreads, final Callback callback) throws IOException {
  ...
  private void loadLabels(AssetManager assetManager, String path) {
    ...
    try {
      ...
      labelMap = new float[1][labels.size()];
      br.close();
    } catch (IOException e) {
 ...
 ByteBuffer feedInputTensorSignal(String path, float mean, float std) throws IOException {
    ...
    ByteBuffer sensData = ByteBuffer.allocateDirect(1 * inputSize * inputSize * inputChannels * bytePerChannel);
    sensData.order(ByteOrder.nativeOrder());
    ...
    for (int i = 0; i < inputSize; ++i) {
      for (int j = 0; j < inputSize; ++j) {
        ...
        if (tensor.dataType() == DataType.FLOAT32) {
          sensData.putFloat((((pixelValue >> 16) & 0xFF) - mean) / std);
          sensData.putFloat((((pixelValue >> 8) & 0xFF) - mean) / std);
          sensData.putFloat(((pixelValue & 0xFF) - mean) / std);
        } else {
          sensData.put((byte) ((pixelValue >> 16) & 0xFF));
          sensData.put((byte) ((pixelValue >> 8) & 0xFF));
          sensData.put((byte) (pixelValue & 0xFF));
        }
      }
    }
...
@ReactMethod
  private void runModelOnSignal(final String path, final float mean, final float std, final int numResults,
                               final float threshold, final Callback callback) throws IOException {

    tfLite.run(feedInputTensorSignal(path, mean, std), labelMap);

    callback.invoke(null, GetTopN(numResults, threshold));
  }

  @ReactMethod
  private void detectObjectOnSignal(final String path, final String model, final float mean, final float std,
    ...
    ByteBuffer sensData = feedInputTensorSignal(path, mean, std);
  ...
  @ReactMethod
  private void runSegmentationOnRawSignal(final String path, final float mean, final float std, final ReadableArray labelColors,
    ...
    tfLite.run(feedInputTensorSignal(path, mean, std), output);
  ...
  private void runPoseNetOnImage(final String path, final float mean, final float std, final int numResults,
    ...
    ByteBuffer sensData = feedInputTensorSignal(path, mean, std);
    Object[] input = new Object[]{sensData};
